...
import javax.persistence.Persistence;
	...
	public class ResourceDAO implements Serializable {
	    ...
	    public void create(Resource vm) throws PreexistingEntityException, Exception {
	    ...
	    public void edit(Resource vm) throws NonexistentEntityException, Exception {
	        EntityManager em = null;
	        try {
	            em = getEntityManager();
	            em.getTransaction().begin();
...
*/
	    public List<Resource> findVMEntities() {
	        return findVMEntities(true, -1, -1);
	    }

	    public List<Resource> findVMEntities(int maxResults, int firstResult) {
	        return findVMEntities(false, maxResults, firstResult);
	    }

	    private List<Resource> findVMEntities(boolean all, int maxResults, int firstResult) {
	        EntityManager em = getEntityManager();
	        try {
	        	System.out.println("start");
	        	Query q = em.createQuery("SELECT v FROM Resource v"); 
	    ...
	    public int getVirtualMachineCount() {
	    	EntityManager em = getEntityManager();
	        try {
	            return ((Long) em.createQuery("select count(v) from Resource v").getSingleResult()).intValue();
	        } finally {
	            em.close();
	        }
	    }
