...
public class AnalyzeAgentPORI extends Agent {
	...
	protected void setup() {
		...
		try {
			...
			dfd.setName(new AID("AnalyzeAgentPORI", AID.ISLOCALNAME));
		...
		addBehaviour(new CyclicBehaviour(this) {
			public void action() {
				...
				if (msgSendorAgent != null) 
	        	{
					...
					if(Nb_VMs ==1 )
					{
						System.out.print("*****************Run 1 VM RORI***************");
...
// *****************Reactive approach Scale In ***************
						
						if(cpuUsage <= CPUThdIN)
						{
							//	Thread.sleep(18000);
							if(tampon!=0)
							{
								System.out.println("the second detection of under-used state");
							...
							{
								tampon=1;
								System.out.println("the first detection of under-used state");
							}
								
						}
						if(cpuUsage > CPUThdIN && tampon!=0)
							tampon=0;
						//******************************************************
