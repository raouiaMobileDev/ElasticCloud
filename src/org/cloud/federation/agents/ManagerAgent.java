...
import org.cloud.federation.nginx.ConfigPound;
import org.cloud.federation.nginx.StartPound;
						...
						*/
						...
						if(msg.getContent().contains("scale_in"))
						{
							...
							try {
								...
								StartPound.stop();
								if(ConfigPound.openFile()== true)
								{
									boolean add=ConfigPound.removeServer("192.168.25.151");
									System.out.println(add);
									boolean save=ConfigPound.saveFile();
								//	Thread.sleep(1000);
								}
								} else if(msg.getContent().contains("3"))
								{
									...
									if(ConfigPound.openFile()== true)
									{
										boolean add=ConfigPound.removeServer("192.168.25.152");
										System.out.println(add);
										boolean save=ConfigPound.saveFile();
									//	Thread.sleep(1000);
									}
						...
						try {
							...
							if(ConfigPound.openFile()== true)
							{
								boolean add=ConfigPound.addServer("192.168.25.151","8080");
								System.out.println(add);
								boolean save=ConfigPound.saveFile();
							}
							} else if(msg.getContent().contains("3"))
							{
								...
								if(ConfigPound.openFile()== true)
								{
									boolean add=ConfigPound.addServer("192.168.25.152","8080");	
									System.out.println(add);
									boolean save=ConfigPound.saveFile();
								}
							}
