...
public class AnalyzeAgentROPIPRAlways extends Agent {
	...
	int windowsTime=7;
	int tampon=0;
	Vector<Double>  vCPU = new Vector<Double>();
		...
		addBehaviour(new CyclicBehaviour(this) {
			public void action() {
				...
				if (msgSendorAgent != null) 
	        	{
					...
					//************* proactive every time a new CPU value comes ****************
					vCPU.add(cpuUsage);
					vTime.add((double) (time));
					time=time+15;
					//**************************************************************************
					...
					if(Nb_VMs==2)
					{
						...
						if(vCPU.size()>=windowsTime)
						{
							
							double[] xx = new double [windowsTime];
							double[] yy = new double [windowsTime];
							
							for(int i=0; i<windowsTime; i++)
							{
								xx[i]=vTime.get(vTime.size()-(windowsTime-i));
								yy[i]=vCPU.get(vCPU.size()-(windowsTime-i));
							}
							
						//double[] yy = {vTime.get(vTime.size()-windowsTime-0), vTime.get(vTime.size()-windowsTime-1), vTime.get(vTime.size()-windowsTime-2), vTime.get(vTime.size()-windowsTime-3), vTime.get(vTime.size()-windowsTime-4), vTime.get(vTime.size()-windowsTime-5),vTime.get(vTime.size()-windowsTime-6)};
					    //double[] yy = {vCPU.get(vCPU.size()-windowsTime-0), vCPU.get(vCPU.size()-windowsTime-1), vCPU.get(vCPU.size()-windowsTime-2), vCPU.get(vCPU.size()-windowsTime-3), vCPU.get(vCPU.size()-windowsTime-4), vCPU.get(vCPU.size()-windowsTime-5),vCPU.get(vCPU.size()-windowsTime-6)};
						//	double[] xx = {15, 30, 45, 60, 75, 90,105,120,135,150,165,180,195 };
					    //	double[] yy = { 20.8, 15.0, 12.8, 12.0, 13.0, 20.1, 26.1, 28.3, 30.5, 20.7, 14.3, 20.3, 22.5};
							
							PolynomialRegression regression = new PolynomialRegression(xx, yy, 2);
							System.out.println("the predicted xx time="+xx[xx.length-1]+15.00);
				            double forecastValue=regression.predict(xx[xx.length-1]+15.00);
				            System.out.println("the predicted value="+forecastValue);
					...
					if(Nb_VMs==3)
					{
						...
						if(vCPU.size()>=windowsTime)
						{
							...
							PolynomialRegression regression = new PolynomialRegression(xx, yy, 2);
				            double forecastValue=regression.predict(xx[xx.length-1]+15);
				            System.out.println("the predicted value="+forecastValue);
							if(forecastValue <= CPUThdIN)
							{
