...
public class AnalyzeAgentPOPI extends Agent {
	...
	protected void setup() {
		...
		try {
			...
			dfd.setName(new AID("AnalyzeAgentPOPI", AID.ISLOCALNAME));
		...
		addBehaviour(new CyclicBehaviour(this) {
			public void action() {
				...
				//	System.out.println("***********sender********"+ msgSendorAgent.getSender().getName() +" info  "+msgSendorAgent.getSender().toString());
					...
					System.out.println("context message= "+MonitorAgentContent);
					...
					if(Nb_VMs==2)
					{
						...
						if(ExponentialSmoothing.observedValues.size()%windowsTime==0 )
						{
							...
							//****Proactive approach Scale IN ****
							else if(forecastValue <= CPUThdIN)
							{
		                        ACLMessage msgr = new ACLMessage(ACLMessage.CFP);   
