...
public class AnalyzeAgentROPIAlways extends Agent {
			...
			public void action() {
				...
				if (msgSendorAgent != null) 
	        	{
					...
					if(Nb_VMs==2)
					{
						...
						{
							System.out.println("forecast available");
							double[] yy = new double [windowsTime];
							
							for(int i=0; i<windowsTime; i++)
								yy[i]=vCPU.get(vCPU.size()-(windowsTime-i));
							
				            double forecastValue=ExponentialSmoothing.ES(yy);
				            System.out.println("forecasted value="+ forecastValue);
